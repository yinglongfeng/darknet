...
layer make_yolo_layer(int batch, int w, int h, int n, int total, int *mask, int classes, int max_boxes)
{
    ...
    //TODO 46
    l.outputs = h*w*n*(classes + 14 + 2 + 1);
    l.inputs = l.outputs;
    l.max_boxes = max_boxes;
    //TODO 47
    l.truths = l.max_boxes*(14 + 2 + 1);    // 90*(4 + 1);
...
void resize_yolo_layer(layer *l, int w, int h)
{
    ...
    l->h = h;
    //TODO 48
    l->outputs = h*w*l->n*(l->classes + 14 + 2 + 1);
    l->inputs = l->outputs;
...
box get_yolo_box(float *x, float *biases, int n, int index, int i, int j, int lw, int lh, int w, int h, int stride)
{
    ...
    b.lx6 = (i + x[index + 14*stride] )  / lw;
    b.ly6 = (j + x[index + 15*stride] )  / lh;
...
ious delta_yolo_box(box truth, float *x, float *biases, int n, int index, int i, int j, int lw, int lh, int w, int h, float *delta, float scale, int stride, float iou_normalizer, IOU_LOSS iou_loss, int accumulate, float max_delta)
{
    ...
    float tlx6 = (truth.lx6*lw - i)  ;
    float tly6 = (truth.ly6*lh - j)  ;
...
//    float land_scale = 1.0 ;
    ...
    if (truth.lx1 != -1 )
    {
        ...
        delta[index + 14 * stride] += scale * (tlx6 - x[index + 14 * stride]) * land_scale;
        delta[index + 15 * stride] += scale * (tly6 - x[index + 15 * stride]) * land_scale;
    ...
    else
    {
        ...
        delta[index + 14 * stride] = 0;
        delta[index + 15 * stride] = 0;
    }
    return all_ious;
}
...
void averages_yolo_deltas(int class_index, int box_index, int stride, int classes, float *delta)
{
	...
	//TODO 51
        ...
        delta[box_index + 14 * stride] /= classes_in_one_box;
        delta[box_index + 15 * stride] /= classes_in_one_box;
    }
}
...
static int entry_index(layer l, int batch, int location, int entry)
{
    ...
    int loc = location % (l.w*l.h);
     //TODO 52
    return batch*l.outputs + n*l.w*l.h*(14 + 2+l.classes+1) + entry*l.w*l.h + loc;
}
...
void forward_yolo_layer(const layer l, network_state state)
{
    ...
    for (b = 0; b < l.batch; ++b) {
	    ...
	    //TODO 53
            index = entry_index(l, b, n*l.w*l.h, 14+2);
    ...
    *(l.cost) = 0;
    for (b = 0; b < l.batch; ++b) {
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    ...
                    // fprintf( stderr, "box pred w h: %f, %f \n", pred.w, pred.h);
                    // fprintf( stderr, "box pred x y: %f, %f \n", pred.x, pred.y);
                    // fprintf( stderr, "box pred p0: %f, %f \n", pred.lx1, pred.ly1);
                    // fprintf( stderr, "box pred p4: %f, %f \n", pred.lx5, pred.ly5);
                    // fprintf( stderr, "box pred p5: %f, %f \n", pred.lx6, pred.ly6);
			...
			//TODO 54
                        box truth = float_to_box_stride(state.truth + t*(14 + 2 + 1) + b*l.truths, 1);
                        int class_id = state.truth[t*(14 + 2 + 1) + b*l.truths + 14 + 2];
			...
			//TODO 55
                        int class_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 14 + 1 + 2);
                        int obj_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 14+2);
			...
			//TODO 58
                        int class_id = state.truth[best_t*(14 + 2 + 1) + b*l.truths + 14 + 2];
			...
			//TODO 59
    			...
    			//TODO 60
                        box truth = float_to_box_stride(state.truth + best_t*(14 + 2 + 1) + b*l.truths, 1);
		...
		//TODO 64
                int obj_index = entry_index(l, b, mask_n*l.w*l.h + j*l.w + i, 14+2);
                avg_obj += l.output[obj_index];
                l.delta[obj_index] = class_multiplier * l.cls_normalizer * (1 - l.output[obj_index]);
		//TODO 65
                int class_index = entry_index(l, b, mask_n*l.w*l.h + j*l.w + i, 14 + 2 + 1);
        ...
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    int box_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 0);
		    //TODO 69
                    int class_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 14 +2 + 1);
    ...
    for (b = 0; b < l.batch; ++b) {
        for (j = 0; j < l.h; ++j) {
		    ...
		    //TODO 70
                    ...
                    no_iou_loss_delta[index + 14 * stride] = 0;
                    no_iou_loss_delta[index + 15 * stride] = 0;
            ...
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    ...
                    for (int i = 0 ; i< l.classes + 1 ; i++){
                        no_iou_class_loss_delta[index + (14 + 2 + i ) * stride] = 0;
                    }
        ...
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    ...
                    no_land_class_loss_delta[index + 14 * stride] = 0;
                    no_land_class_loss_delta[index + 15 * stride] = 0;
...
void correct_yolo_boxes(detection *dets, int n, int w, int h, int netw, int neth, int relative, int letter)
{
	...
	//TODO 77
        ...
        b.lx6 = (b.lx6 - deltaw / 2. / netw) / ratiow;
        b.ly6 = (b.ly6 - deltah / 2. / neth) / ratioh;
...
int yolo_num_detections(layer l, float thresh)
{
    ...
    for (i = 0; i < l.w*l.h; ++i){
        for(n = 0; n < l.n; ++n){
	    //TODO 79
            int obj_index  = entry_index(l, 0, n*l.w*l.h + i, 14+2);
...
int yolo_num_detections_batch(layer l, float thresh, int batch)
{
    ...
    for (i = 0; i < l.w*l.h; ++i){
        for(n = 0; n < l.n; ++n){
	    //TODO 80
            int obj_index  = entry_index(l, batch, n*l.w*l.h + i, 14+2);
            if(l.output[obj_index] > thresh){
                ++count;
            }
        }
...
int get_yolo_detections(layer l, int w, int h, int netw, int neth, float thresh, int *map, int relative, detection *dets, int letter)
{
    ...
    for (i = 0; i < l.w*l.h; ++i){
	    ...
	    //TODO 82
		    ...
		    //TODO 83
                    int class_index = entry_index(l, 0, n*l.w*l.h + i, 14 +2 + 1 + j);
    ...
    for (i = 0; i < l.w*l.h; ++i){
	    ...
	    //TODO 84
            ...
            if (objectness > thresh) {
                ...
                dets[count].classes = l.classes;
                for (j = 0; j < l.classes; ++j) {
                    int class_index = entry_index(l, batch, n*l.w*l.h + i, 14 +2 + 1 + j);
                    float prob = objectness*predictions[class_index];
                    dets[count].prob[j] = (prob > thresh) ? prob : 0;
                }
                ++count;
